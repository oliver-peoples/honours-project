...
np.savetxt('calibration_mca_coefficients.csv', np.array(calibration_mca_coefficients), delimiter=',')
...
energy_counts, _, _, integration_time = loadAsciiSpectrum('raw-data/bg.Spe')
...
energy_counts[:,0] = calibration_mca_coefficients[0] + calibration_mca_coefficients[1] * energy_counts[:,0] + calibration_mca_coefficients[2] * energy_counts[:,0] ** 2
plt.plot(energy_counts[:,0], energy_counts[:,1], c='b', linewidth=0.5, alpha=0.5)
energy_counts[:,1] = scipy.ndimage.gaussian_filter1d(energy_counts[:,1] / 1., 1)

plt.plot(energy_counts[:,0], energy_counts[:,1], c='b', linewidth=0.75)
...
plt.tight_layout()
plt.gca().set_aspect(aspect)
plt.savefig(f'figures/background.png', dpi=400, bbox_inches='tight')
plt.close()