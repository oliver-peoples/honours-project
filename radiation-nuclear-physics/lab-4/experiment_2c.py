...
spectra_files = sorted(glob.glob('raw-data/DECAY_2C_0*.Spe'))
...
for spectra_file, idx in zip(spectra_files, range(num_spectra_files)):
    ...
    plt.plot(energy_counts[:,0], energy_counts[:,1], c='b', linewidth=0.75, alpha = 0.6 - 0.4 * (float(idx) / float(num_spectra_files - 1)))
    ...
    totals[idx,0] = epoch
    totals[idx,1] = np.sum(energy_counts[:,1])
...
plt.savefig(f'figures/decay_2c.png', dpi=400, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.gca().set_aspect(aspect)
plt.savefig(f'figures/decay_2c_roi.png', dpi=400, bbox_inches='tight')
plt.close()