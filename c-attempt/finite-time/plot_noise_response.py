...
def main() -> None:
    ...
    cbar.set_label(r"$Relative Prevalence$", fontsize=10, rotation=-90, labelpad=10)
    plt.xlabel(r'$\eta\mathrm{\;[\%]}$', fontsize=10)
    plt.ylabel(r'$\bar{w}_{eff} \mathrm{\;[\sigma]}$', fontsize=10)
    plt.tight_layout()
    ...
    # cbar.ax.tick_params(labelsize=8)
    # cbar.set_label(r"$Relative Prevalence$", fontsize=10, rotation=-90, labelpad=15)
    ...
    # # 
    # plt.xlabel(r"$\eta$", fontsize=10, labelpad=1)
    # plt.ylabel(r"$\bar{w}_{eff}$", fontsize=10, labelpad=3)
