...
void mainSimple(void)
{
    ...
    savePoints("avg-finite-time/core_locations.csv", core_locations);
    saveIndexes("avg-finite-time/g2_capable_indexes.csv", g2_capable_idx);
    ...
    savePoints("avg-finite-time/emitter_xy.csv", emitter_xy);
    ...
    double avg_w_bar_eff = 0;
    double avg_e1_error = 0;
    double avg_e2_error = 0;

    std::default_random_engine generator(std::random_device{}());

    for (int repeat_idx = 0; repeat_idx < REPEATS; repeat_idx++)
    {
        ...
        savePoints("avg-finite-time/data/iter_" + std::to_string(repeat_idx) + "_emitter_guesses.csv", emitter_guesses);
    
        savePoints("avg-finite-time/data/iter_" + std::to_string(repeat_idx) + "_x1s_convex_hull.csv", x1s_convex_hull);
        savePoints("avg-finite-time/data/iter_" + std::to_string(repeat_idx) + "_x2s_convex_hull.csv", x2s_convex_hull);

        savePoints("avg-finite-time/data/iter_" + std::to_string(repeat_idx) + "_x1s.csv", x1s);
        savePoints("avg-finite-time/data/iter_" + std::to_string(repeat_idx) + "_x2s.csv", x2s);

        avg_w_bar_eff += 0.5 * (e1_weff + e2_weff);
        avg_e1_error += sqrt(mean_e1_error_x * mean_e1_error_x + mean_e1_error_y * mean_e1_error_y);
        avg_e2_error += sqrt(mean_e2_error_x * mean_e2_error_x + mean_e2_error_y * mean_e2_error_y);
        ...
        printf("> %i: fit quality overall: %0.4f, %0.4f, SF: %f\n", repeat_idx, 0.5 * (fitQuality(x1s) + fitQuality(x2s)), 0.5 * (e1_weff + e2_weff), 1./ (0.5 * (e1_weff + e2_weff)));
        // printf(
    ...
    avg_w_bar_eff /= (double)REPEATS;
    avg_e1_error /= (double)REPEATS;
    avg_e2_error /= (double)REPEATS;

    printf("> average w_bar_eff, SF: %f, %f\n", avg_w_bar_eff, 1/avg_w_bar_eff);
    printf("> average error e1, e2: %f, %f\n", avg_e1_error, avg_e2_error);
}