...
cbar = plt.colorbar(pad=0.01)
cbar.ax.tick_params(labelsize=18)
cbar.set_label(r"$I_{pl}\left(\rho,\phi\right)$", fontsize=24, rotation=-90, labelpad=28)
# cbar.set_ticks
plt.gca().set_aspect(1)
plt.tight_layout()
plt.savefig(f'gl-modes/p_{order}_l_{index}_i_pl.png', dpi=600, bbox_inches='tight')
plt.close()